...
function MeteogramOpenMeteo({
  ...
  const plot = useMemo(() => {
    ...
    const height = margin.top + 3 * rowH + 2 * gap + margin.bottom;
    ...
    <div className="meteogram-card">
      <div className="meteogram-header">
        <div>
          <h2>Meteogram</h2>
          <p>
            Open-Meteo hourly data @ lat {latitude}, lon {longitude}
          </p>
        </div>
      ...
      <div className="meteogram-scroll">
        <svg viewBox={`0 0 ${width} ${height}`} className="meteogram-svg" role="img" aria-label="Meteorological meteogram">
          ...
          {/* Bottom date labels */}
          {dateLabels.map((d) => (
            <text key={d.date} x={d.x} y={height - 28} textAnchor="middle" fontSize="24" fontWeight="700" fill="#111">
              {d.date}
            </text>
          ))}
        </svg>
...
export default MeteogramOpenMeteo
